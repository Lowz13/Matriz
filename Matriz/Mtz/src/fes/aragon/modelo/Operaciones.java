...
public class Operaciones {
    ...
    public static int[] BuscarCero(double mtz[][],int renglon,int columna,int r,int c){
        int eR=-1,eC=-1;
        double aux=0;
        for (int i = r; i < renglon; i++) {
            if (i!=r) {
                if (mtz[r][c] == mtz[i][c] || mtz[r][c] == (-1 * mtz[i][c]) ) {
                    ...
                    eC = c;
                    break;
                } else if ((mtz[r][c] % mtz[i][c]) == 0 || (mtz[i][c] % mtz[r][c]) == 0) {
                    eR = i;
                    eC = c;
                    break;
                }
    ...
    public double[][] ResolverCero(double mtz[][],int r,int c,int er,int ec,int l,int columna){
        double aux;
        //pibotea con la diagonal
        if (ec==-1 && er==-1) {
            for (int i = 0; i < columna; i++) {
                mtz[r][i] = (-mtz[l][i] * mtz[r][i]) + mtz[r][i];
            }
        }else {
            //suma
